...
import { fetchRecrProcess } from "../../../states/actions/fetchRecrProcess";
...
const RecruitmentProcess = () => {
  ...
  const data = useSelector((state) => state.fetchRecrProcess.content);
  const isLoading = useSelector((state) => state.fetchRecrProcess.isLoading);
  const { content, sec2s } = data;
  const [selectedTab, setSelectedTab] = useState(0);

  useEffect(() => {
    dispatch(fetchRecrProcess());
  }, []);
